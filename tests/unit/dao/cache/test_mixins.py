...
def secrets_client():
    ...
    client.get_secret_value.return_value = {
        'SecretString': json.dumps({'username': 'user_from_secret', 'password': 'p'})
    }
    return client
...
def test_fallback_to_ssm_username_when_secret_omits_username(
    app_prefix, ssm_client, secrets_client_no_username, redis_client
):
    ...
    with patch('cloudshortener.dao.cache.mixins.redis.Redis', autospec=True) as redis_mock:
        ...
        ElastiCacheClientMixin(
            prefix=app_prefix, ssm_client=ssm_client, secrets_client=secrets_client_no_username
        )
