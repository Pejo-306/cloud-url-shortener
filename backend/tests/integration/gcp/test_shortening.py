...
from tests.integration.gcp.schemas import app_prefix


class TestGcpShortening:
    def test_e2e_shortening(self, infra: Infrastructure):
        # 0. Set target URL
        ...
        with infra.identity_platform.authenticate() as id_token:
            response = requests.post(
                url=infra.backend.api.shorten_url,
            ...
            response = requests.get(
                url=infra.backend.api.redirect_url(shortcode),
    ...
    def test_unauthorized_shortening(self, infra: Infrastructure):
        # 0. Set target URL
    ...
    def test_nonexistent_shortcode(self, infra: Infrastructure, project: GcpProject, data_stores: DataStores):
        # 1. Generate a random shortcode
        # 2. Send a GET request to the redirect URL endpoint
        # 3. Assert that the response is 400
        key_prefix = app_prefix(project)
    ...
    def test_non_conflict_shortening(self, infra: Infrastructure):
        # 0. Set target URL
        # 1. Authenticate with Identity Platform
        # 2. Send a POST request to the shorten URL endpoint
        ...
        with infra.identity_platform.authenticate() as id_token:
            ...
            shortcode1 = response1.json()['shortcode']
            shortcode2 = response2.json()['shortcode']
